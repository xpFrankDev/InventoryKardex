 ...
 * @author Family
 */
public class panelParametroColor extends javax.swing.JPanel {

    ArrayList<tipoColor> colores = new ArrayList();
    int valor = -1;
    String estado = "Activo";
    public boolean agregar=false;
    public boolean editar=false;
    public panelParametroColor() {
        initComponents();
        //reset();
    }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Configuración de Colores");
    ...
    private void editarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editarBtnActionPerformed
        ...
        if (fila >= 0 && !id.equals("-")) {
            ...
            for (tipoColor color : colores) {
                if (idint == color.getId()) {
                    idTxt.setText(color.getId() + "");
                    nombreTxt.setText(color.getNombre());
                    if (color.getEstado().equalsIgnoreCase("activo")) {
                        activoRb.setSelected(true);
                    } else {
    ...
    private void guardarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_guardarBtnActionPerformed
        if (validarCamposLlenos() == true) {
            ...
            if (valor == 0) {//nuevo
                ...
                for (tipoColor color : colores) {
                    if (color.getNombre().equalsIgnoreCase(nombre)) {
                        repetido = true;
                    }
                }
                if (repetido == false) {
                    int respuesta = new tipoColor().newColor(nombre);
                    if (respuesta > 0) {
                ...
                int respuesta = new tipoColor().updateColor(id, nombre, estado);
                if (respuesta > 0) {
    ...
    public void llenarColores() {
        ...
        colores.clear();
        colores = (ArrayList) new tipoColor().Listar("").clone();
        for (tipoColor color : colores) {
            modelo.addRow(new Object[]{
                color.getId(), color.getNombre(), color.getEstado()
            });
        }
        if (colores.size() < 1) {
            modelo.addRow(new Object[]{
    ...
    public void llenarColores(String estadox) {
        DefaultTableModel modelo = (DefaultTableModel) tabla.getModel();
        modelo.setRowCount(0);
        colores.clear();
        colores = (ArrayList) new tipoColor().Listar(estadox).clone();
        for (tipoColor color : colores) {
            modelo.addRow(new Object[]{
                color.getId(), color.getNombre(), color.getEstado()
            });
