...
public class tipoColor {
    ...
    public tipoColor(int id, String nombre, String estado) {
        this.id = id;
        this.nombre = nombre;
        this.estado = estado;
    }
      ...
      public ArrayList Listar(String valor) {
        ArrayList<tipoColor>lista = new ArrayList();

        sql = "call  `sp.ConsColorxEstado` (?)";
        try (Connection con = new ConectarCloudcPanel("corporacionagama_mainbigmedic").getCon()) {
            ...
            while(rs.next()){
                ...
                lista.add(new tipoColor(rs.getInt("id"),rs.getString("nombre"),rs.getString("estado")));
    ...
    public int newColor(String nombrex) {
       int valor = 0;
        sql = "call  `sp.newColor` (?)";
        try (Connection con = new ConectarCloudcPanel("corporacionagama_mainbigmedic").getCon()) {
            pst = con.prepareCall(sql);
      ...
      public int updateColor(int idx, String nombrex,String estadox) {
       int valor = 0;
        sql = "call  `sp.updateColor` (?,?,?)";
        try (Connection con = new ConectarCloudcPanel("corporacionagama_mainbigmedic").getCon()) {
            pst = con.prepareCall(sql);
            pst.setInt(1,idx);
