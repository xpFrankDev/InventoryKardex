...
public class Familia {
    ...
    public Familia(int id, String nombre, String estado) {
        this.id = id;
        this.nombre = nombre;
        this.estado = estado;
    }
     ...
     public ArrayList Listar(String valor) {
        ...
        sql = "call `sp.ConsFamiliaxEstado` (?)";
        try (Connection con = new ConectarCloudcPanel("corporacionagama_mainbigmedic").getCon()) {
            pst = con.prepareCall(sql);
            pst.setString(1,valor);
            rs = pst.executeQuery();
            while(rs.next()){
                 lista.add( new Familia(rs.getInt("id"),rs.getString("nombre"),rs.getString("estado")));
            }
        } catch (Exception e) {
       ...
       public int newFamilia(String nombrex) {
        int valor = 0;
        sql = "call  `sp.newFamilia` (?)";
        try (Connection con = new ConectarCloudcPanel("corporacionagama_mainbigmedic").getCon()) {
            pst = con.prepareCall(sql);
    ...
    public int updateFamilia(int idx, String nombrex, String estadox) {
        int valor = 0;
        sql = "call  `sp.updateFamilia` (?,?,?)";
        try (Connection con = new ConectarCloudcPanel("corporacionagama_mainbigmedic").getCon()) {
            pst = con.prepareCall(sql);
            pst.setInt(1, idx);
